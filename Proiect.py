...
#   Observăm că există valori lipsă
# Pentru tratarea problemei valorilor lipsă, grupăm țările în funcție de HDI. 
# Țările cu un indice al dezvoltării umane asemănător au de obicei valori similare pentru restul variabilelor

# Pasul 1. Creare grupe după HDI
...
# Pasul 3. Verificăm dacă mai există valori lipsă
print("\n----- Valori lipsă pe coloană -----")
print(df.isnull().sum())

# Din moment ce acum există o singură valoare lipsă în întreaga bază de date, o putem elimina, 
# deoarece această observație nu are un impact puternic asupra analizei noastre.
...
#    Observăm outlieri în cazul variabilelor *Populație* și *NrNasteri*
...
# Histograma originală - Populatie
...
# Histograma transformată - Populatie
...
# Histograma originală - NrNasteri
...
# Histograma transformată - NrNasteri
...
# Alegem ca variabile explicative rata populației cu acces la electricitate, populația (valoarea logaritmată), 
# numărul de nașteri (valoarea logaritmată), speranța de viață, numărul așteptat de ani de școlarizare. 
# Nu includem variabilele *natalitate* și *nivel de dezvoltare*, acestea fiind derivate din populație și numărul de nașteri, respectiv derivată din HDI
...
X = df[['AccesElectricitate%', 'Populatie_log', 'NrNasteri_log', 'LifeExpectancyAtBirth', 'ExpectedYearsOfSchooling']]
...
# În urma aplicării modelului OLS pe setul nostru de date, obținem un R-squared = 0.9 și Prob (F-statistic) = 1.36e-84,
# ceea ce arată o calitate crescută a modelului.
# Cu excepția variabilei AccesElectricitate%, toate celelalte variabile explicative sunt semnificative statistic 
# pentru un prag de 0.05, așadar variabila menționată poate fi exclusă din modelul OLS.
# De asemenea, valoarea testului Durbin-Watson = 2.037 (foarte apropiată de 2) arată că reziduurile nu sunt autocorelate
# Trebuie ținut cont și de faptul că reziduurile nu sunt distribuite normal (testul Jarque-Bera are o valoare foarte crescută)
...
# Se observă o diferență de aproape 0,1 între R-squared pentru setul de antrenare și cel de test, 
# iar valorile pentru RMSE nu diferă nici ele semnificativ, ceea ce indică faptul că nu este prezent cazul de overfitting.
...
# Am aplicat Regresia Lasso pentru a identifica cele mai importante variabile explicative. 
# Acest model penalizează coeficienții irelevanți, împingându-i spre zero, ajutând astfel la selecția caracteristicilor relevante.
